...
app.post('/api/jee/download', (req, res) => {
  ...
  if (type === 'question_paper') {
    ...
    txtContent += `\n========================================================\n`;
    txtContent += `Generated by Ishwar Hiremath Academy - Educuria Portal 2026\n`;
    txtContent += `========================================================\n`;
...
app.get('/api/puc/papers', (req, res) => {
  ...
  const subjects = ['Mathematics', 'Physics', 'Chemistry', 'Accountancy', 'English'];
...
app.post('/api/puc/download', (req, res) => {
  ...
  if (type === 'question_paper') {
    ...
    const normSubject = subject.toLowerCase();
    const qData = PUC_PAPERS_DATABASE.questions[normSubject] || PUC_PAPERS_DATABASE.questions.mathematics;
    ...
    txtContent += `SUBJECT: ${subject.toUpperCase()}\n`;
    txtContent += `PAPER NUMBER: Paper ${paperNum}\n`;
    txtContent += `DURATION: 3 Hours 15 Minutes           MAX MARKS: 80\n\n`;
    txtContent += `------------------ GENERAL INSTRUCTIONS ------------------\n`;
    qData.instructions.forEach((ins: string, idx: number) => {
      txtContent += `${idx + 1}. ${ins}\n`;
    });
    ...
    return res.json({
      filename: `KSEAB_2PUC_${year}_${subject}_Paper_${paperNum}.txt`,
      title: `KSEAB II PUC ${year} - ${subject} (Paper ${paperNum})`,
      content: txtContent
    });
  }
